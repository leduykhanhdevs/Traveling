...
const findUserIdByClerkId = async (clerkUserId: string): Promise<string> => {
  ...
  if (!user) {
    throw new AppError('USER_NOT_FOUND', 'Create a WanderAI profile before using SOS.', 404);
  }
...
export const sendSOSAlert = async (input: SOSAlertInput): Promise<SOSAlertConfirmation> => {
  ...
  if (!user) {
    throw new AppError('USER_NOT_FOUND', 'Create a WanderAI profile before sending SOS.', 404);
  }
...
export const registerDeviceToken = async (
  ...
  if (!user) {
    throw new AppError('USER_NOT_FOUND', 'Create a WanderAI profile before registering devices.', 404);
  }
