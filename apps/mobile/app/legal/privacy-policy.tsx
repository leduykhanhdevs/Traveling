...
const legalUrl = (): string => {
  ...
  return `${baseUrl.replace(/\/$/, '')}/legal/privacy-policy`;
};
...
export default function PrivacyPolicyScreen(): JSX.Element {
  ...
  const openPolicy = useCallback(async (): Promise<void> => {
    setOpening(true);
    setError(null);
    ...
    try {
      ...
      const message =
        caught instanceof Error ? caught.message : 'Unable to open the Privacy Policy.';
      setError(message);
    } finally {
      setOpening(false);
    }
      ...
      <View className="flex-1 justify-center">
        <GlassCard>
          <View className="gap-4">
            <View>
              <Text className="font-inter-bold text-3xl text-white">{t('legal.privacyPolicy')}</Text>
            ...
            <PrimaryButton
              label={t('legal.openPrivacyPolicy')}
              icon={ExternalLink}
              loading={opening}
              onPress={() => {
                void openPolicy();
