import type { CommunityFilters, CommunityReview } from '@traveling/shared';
...
const userByClerkId = async (clerkUserId: string): Promise<{ id: string; email: string }> => {
  ...
  if (!user) {
    throw new AppError(
      'USER_NOT_FOUND',
      'Create a Traveling profile before using community features.',
      404,
    );
  }
...
export const createCommunityReview = async (input: CreateReviewInput): Promise<CommunityReview> => {
  ...
  if (!user) {
    throw new AppError('USER_NOT_FOUND', 'Create a Traveling profile before posting a review.', 404);
  }
...
export const followTraveler = async (
  ...
  if (!follower) {
    throw new AppError(
      'USER_NOT_FOUND',
      'Create a Traveling profile before following travelers.',
      404,
    );
  }
