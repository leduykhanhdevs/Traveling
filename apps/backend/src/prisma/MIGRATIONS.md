...
```bash
npm run prisma:migrate --workspace @traveling/backend -- --name YYYYMMDDHHMMSS_description_snake_case
```
...
npm run typecheck --workspace @traveling/backend
```
...
```bash
npm run prisma:deploy --workspace @traveling/backend
```
...
```bash
npm run db:seed --workspace @traveling/backend
```
