...
describe('utility.controller', () => {
  ...
  describe('success case', () => {
    ...
    it('converts an amount between two currencies', async () => {
      const conversion = {
        from: 'USD',
        to: 'VND',
        amount: 10,
      ...
      convertCurrencyMock.mockResolvedValue(conversion);
      const app = buildTestApp([
        {
          method: 'get',
          path: '/currency/convert',
          handlers: [authenticated('clerk_user_1'), getCurrencyConversion],
        },
      ]);

      const response = await request(app)
        .get('/currency/convert')
        .query({ from: 'usd', to: 'vnd', amount: '10' });

      expect(response.status).toBe(200);
      expect(response.body).toEqual({ success: true, data: conversion });
      expect(convertCurrencyMock).toHaveBeenCalledWith('USD', 'VND', 10);
    });
  ...
  describe('4xx error', () => {
    it('returns validation error for an invalid currency code', async () => {
      const convertCurrencyMock = jest.mocked(convertCurrency);
      const app = buildTestApp([
        {
          method: 'get',
      ...
      const response = await request(app)
        .get('/currency/convert')
        .query({ from: 'US', to: 'VND', amount: '10' });
